...
then
    ...
    mv out/$PATH_OUTPUTS/$ALGORITHM\_{1..2}.out error/$PATH_OUTPUTS/$ALGORITHM\_{1..2}.out
    mv out/$PATH_OUTPUTS/$ALGORITHM\manager.out error/$PATH_OUTPUTS/$ALGORITHM\manager.out
    exit
...
then
    ...
    # If the number of hour is lower than 10, we add 30 minutes
    ...
    # If the number of hour is lower than 12 we need use the "short" partition
...
rm out/$PATH_OUTPUTS/$ALGORITHM\manager.out
submission_manager=$(sbatch -J $PATH_OUTPUTS/$ALGORITHM\manager --dependency=afterany:$JOB_ID_RUN -o out/$PATH_OUTPUTS/$ALGORITHM\manager.out fit_running/stage/manager.sh -jir $JOB_ID_RUN -tt $TRAINING_TYPE -mc $MAIN_CATEGORY -c $CATEGORY -t $TARGET -a $ALGORITHM -nis $N_INNER_SEARCH)
