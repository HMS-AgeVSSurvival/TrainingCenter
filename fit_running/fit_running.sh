...
submission_run=$(sbatch -J $PATH_OUTPUTS/$ALGORITHM\_200M_00:05:00 --partition short --array=1-2 --mem-per-cpu=200M --time 00:05:00 -o out/$PATH_OUTPUTS/$ALGORITHM\_%a.out fit_running/stage/run.sh -tt $TRAINING_TYPE -mc $MAIN_CATEGORY -c $CATEGORY -t $TARGET -a $ALGORITHM -nis $N_INNER_SEARCH)
echo $submission_run
