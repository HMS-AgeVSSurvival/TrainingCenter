...
class TestCategories(unittest.TestCase):
    def test_matching_categories(self):
        ...
        for main_category in ["examination", "laboratory", "questionnaire"]:
            categories_in_sheet = pd.Index(get_col_values(main_category, "category")[3:])
            categories_in_data = pd.Index(os.listdir(f"data/{main_category}")).str.split(".").map(lambda splitted_file: splitted_file[0])
