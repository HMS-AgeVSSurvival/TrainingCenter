...
def update_results_survival(main_category, category, algorithm, target, metrics, training_type, random_state):
    results_updated = False

    category_row = find_cell(main_category + f" {random_state}", category).row
    test_c_index_column = findall_cells(main_category + f" {random_state}", "test C-index")[METRICS_COL_ORDER_SURVIVAL[training_type][target][algorithm]].col
    ...
    if previous_test_c_index is None or float(previous_test_c_index) < metrics["test C-index"]:
        ...
        results_updated = True
        for metric_name in list(metrics.keys()):
            metric_column = findall_cells(main_category + f" {random_state}", metric_name)[METRICS_COL_ORDER_SURVIVAL[training_type][target][algorithm]].col
            update_cell(main_category + f" {random_state}", category_row, metric_column, np.round(metrics[metric_name], 3))
